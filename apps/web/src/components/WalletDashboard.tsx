...
import { shortenAddress, qrUrl, MIN_LAUNCH_SOL } from "@/lib/clientWallet";

export function WalletDashboard() {
  const { publicKey, balance, canLaunch, loading, refresh, exportPrivateKey, importKey, reset } =
    useNarraWallet();
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-10">
      ...
      >
        <span
          className={`w-2 h-2 rounded-full shrink-0 ${canLaunch ? "bg-green-400" : "bg-yellow-400"}`}
        />
        {canLaunch
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-white/35">
              Paste your private key from the extension to use the same wallet here
            </p>
          </div>
          <button
        ...
        {showImport && (
          <form onSubmit={handleImport} className="mt-4">
            <p className="text-xs text-white/40 mb-2">
              In your extension: Wallet tab → Export Private Key → copy the key → paste below
            </p>
            <input
              type="password"
              value={importInput}
        ...
        {!showReset ? (
          ...
          <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5">
            ...
            <p className="text-xs text-red-300/60 mb-4">
              This replaces your current keypair. Make sure you&apos;ve exported your private key first — any
              SOL in this wallet will be inaccessible.
            </p>
